...
public:
  void process(struct DCtoEx dctoEx, struct ExtoMem& extoMem, bool& stall)
  {
    ...
    switch (dctoEx.opCode) {
      ...
      case RISCV_SYSTEM:
        switch (dctoEx.funct3) { // case 0: mret instruction, dctoEx.memValue should be 0x302
          case RISCV_SYSTEM_ENV:
#ifndef __HLS__
            // TODO handling syscall correctly
            // extoMem.result = sim->solveSyscall(dctoEx.lhs, dctoEx.rhs, dctoEx.datac, dctoEx.datad, dctoEx.datae,
            // exit);
#endif
            break;
