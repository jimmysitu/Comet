...
template <> inline double ldexpr32<0>(double d)
{
  return d;
}
...
template <int N> inline void iv_copy(const int* op, int* r)
{
  for (int i = 0; i < N; i++)
    r[i] = op[i];
}
...
template <int N> inline bool iv_equal_zero(const int* op)
{
  for (int i = 0; i < N; i++)
    if (op[i])
      return false;
...
template <> inline bool iv_equal_zero<0>(const int* /*op*/)
{
  return true;
}
...
template <int N> inline bool iv_equal_ones(const int* op)
{
  for (int i = 0; i < N; i++)
    if (~op[i])
      return false;
...
template <> inline bool iv_equal_ones<0>(const int* /*op*/)
{
  return true;
}
...
template <int N1, int N2> inline bool iv_equal(const int* op1, const int* op2)
{
...
template <int B, int N> inline bool iv_equal_ones_from(const int* op)
{
...
template <int B, int N> inline bool iv_equal_zeros_from(const int* op)
{
  if ((B >= 32 * N && op[N - 1] < 0) || (B & 31 && (op[B / 32] >> (B & 31))))
    return false;
...
template <int B, int N> inline bool iv_equal_ones_to(const int* op)
{
...
template <int B, int N> inline bool iv_equal_zeros_to(const int* op)
{
  if ((B >= 32 * N && op[N - 1] < 0) || (B & 31 && (op[B / 32] & ~(all_ones << (B & 31)))))
    return false;
...
template <int N1, int N2, bool greater> inline bool iv_compare(const int* op1, const int* op2)
{
  const int M1   = AC_MAX(N1, N2);
  const int M2   = AC_MIN(N1, N2);
  const int* OP1 = N1 >= N2 ? op1 : op2;
...
template <int N> inline void iv_extend(int* r, int ext)
{
  for (int i = 0; i < N; i++)
    r[i] = ext;
}
...
template <int Nr> inline void iv_assign_int64(int* r, Slong l)
{
  r[0] = (int)l;
  if (Nr > 1) {
    r[1] = (int)(l >> 32);
...
template <int Nr> inline void iv_assign_uint64(int* r, Ulong l)
{
  r[0] = (int)l;
  if (Nr > 1) {
    r[1] = (int)(l >> 32);
...
template <int N1, int N2, int Nr> inline void iv_mult(const int* op1, const int* op2, int* r)
{
  if (Nr == 1)
    r[0] = op1[0] * op2[0];
  else if (N1 == 1 && N2 == 1)
...
template <int N> inline bool iv_uadd_carry(const int* op1, bool carry, int* r)
{
  Slong l = carry;
  for (int i = 0; i < N; i++) {
...
template <> inline bool iv_uadd_carry<0>(const int* /*op1*/, bool carry, int* /*r*/)
{
  return carry;
}
...
template <int N> inline bool iv_add_int_carry(const int* op1, int op2, bool carry, int* r)
{
  if (N == 0)
    return carry;
  if (N == 1) {
...
template <> inline bool iv_add_int_carry<0>(const int* /*op1*/, int /*op2*/, bool carry, int* /*r*/)
{
  return carry;
}
...
template <int N> inline bool iv_uadd_n(const int* op1, const int* op2, int* r)
{
  Ulong l = 0;
  for (int i = 0; i < N; i++) {
...
template <> inline bool iv_uadd_n<0>(const int* /*op1*/, const int* /*op2*/, int* /*r*/)
{
  return false;
}
...
template <int N1, int N2, int Nr> inline void iv_add(const int* op1, const int* op2, int* r)
{
  if (Nr == 1)
    r[0] = op1[0] + op2[0];
  else {
...
template <int N> inline bool iv_sub_int_borrow(const int* op1, int op2, bool borrow, int* r)
{
  if (N == 1) {
...
template <> inline bool iv_sub_int_borrow<0>(const int* /*op1*/, int /*op2*/, bool borrow, int* /*r*/)
{
  return borrow;
}
...
template <int N> inline bool iv_sub_int_borrow(int op1, const int* op2, bool borrow, int* r)
{
  if (N == 1) {
...
template <> inline bool iv_sub_int_borrow<0>(int /*op1*/, const int* /*op2*/, bool borrow, int* /*r*/)
{
  return borrow;
}
...
template <int N> inline bool iv_usub_n(const int* op1, const int* op2, int* r)
{
  Slong l = 0;
  for (int i = 0; i < N; i++) {
    l += (Ulong)(unsigned)op1[i] - (Ulong)(unsigned)op2[i];
...
template <int N1, int N2, int Nr> inline void iv_sub(const int* op1, const int* op2, int* r)
{
  if (Nr == 1)
    r[0] = op1[0] - op2[0];
  else {
...
template <int N> inline bool iv_all_bits_same(const int* op, bool bit)
{
  int t = bit ? ~0 : 0;
  for (int i = 0; i < N; i++)
    if (op[i] != t)
...
template <> inline bool iv_all_bits_same<0>(const int* /*op*/, bool /*bit*/)
{
  return true;
}
...
template <int N, int Nr> void iv_neg(const int* op1, int* r)
{
  Slong l = 0;
...
template <int N, bool S, int Nr> void iv_abs(const int* op1, int* r)
{
  if (S && op1[N - 1] < 0) {
    iv_neg<N, Nr>(op1, r);
  } else {
...
template <int N1, int Num_s, int N2, int Den_s, int Nr> inline void iv_div(const int* op1, const int* op2, int* r)
{
...
template <int N1, int Num_s, int N2, int Den_s, int Nr> inline void iv_rem(const int* op1, const int* op2, int* r)
{
...
template <int N> inline void iv_bitwise_complement_n(const int* op, int* r)
{
  for (int i = 0; i < N; i++)
    r[i] = ~op[i];
}
...
template <int N, int Nr> inline void iv_bitwise_complement(const int* op, int* r)
{
...
template <int N> inline void iv_bitwise_and_n(const int* op1, const int* op2, int* r)
{
  for (int i = 0; i < N; i++)
    r[i] = op1[i] & op2[i];
}
...
template <int N1, int N2, int Nr> inline void iv_bitwise_and(const int* op1, const int* op2, int* r)
{
...
template <int N> inline void iv_bitwise_or_n(const int* op1, const int* op2, int* r)
{
  for (int i = 0; i < N; i++)
    r[i] = op1[i] | op2[i];
}
...
template <int N1, int N2, int Nr> inline void iv_bitwise_or(const int* op1, const int* op2, int* r)
{
  const int M1   = AC_MIN(AC_MAX(N1, N2), Nr);
  const int M2   = AC_MIN(AC_MIN(N1, N2), Nr);
  const int* OP1 = N1 >= N2 ? op1 : op2;
...
template <int N> inline void iv_bitwise_xor_n(const int* op1, const int* op2, int* r)
{
  for (int i = 0; i < N; i++)
    r[i] = op1[i] ^ op2[i];
}
...
template <int N1, int N2, int Nr> inline void iv_bitwise_xor(const int* op1, const int* op2, int* r)
{
  const int M1   = AC_MIN(AC_MAX(N1, N2), Nr);
  const int M2   = AC_MIN(AC_MIN(N1, N2), Nr);
  const int* OP1 = N1 >= N2 ? op1 : op2;
...
template <int N, int Nr> inline void iv_shift_r(const int* op1, unsigned op2, int* r)
{
  unsigned s31    = op2 & 31;
...
template <int N, int Nr, bool S> inline void iv_shift_l2(const int* op1, signed op2, int* r)
{
  if (S && op2 < 0)
...
{
  r[0] = (op2 >= 0) ? (op2 < 32) ? (op1[0] << op2) : 0 : (op2 > -32) ? (op1[0] >> -op2) : (op1[0] >> 31);
}

template <int N, int Nr, bool S> inline void iv_shift_r2(const int* op1, signed op2, int* r)
{
  if (S && op2 < 0)
    iv_shift_l<N, Nr>(op1, -op2, r);
  else
...
{
  r[0] = (op2 >= 0) ? (op2 < 32) ? (op1[0] >> op2) : (op1[0] >> 31) : (op2 > -32) ? (op1[0] << -op2) : 0;
}

template <int N, int Nr, int B> inline void iv_const_shift_l(const int* op1, int* r)
{
  // B >= 0
  if (!B) {
...
template <int N, int Nr, int B> inline void iv_const_shift_r(const int* op1, int* r)
{
  if (!B) {
    const int M1 = AC_MIN(N, Nr);
    iv_copy<M1>(op1, r);
...
template <int N> inline void iv_conv_from_fraction(double d, int* r, bool* qb, bool* rbits, bool* o)
{
  bool b        = d < 0;
  double d2     = b ? -d : d;
...
template <ac_base_mode b> inline int to_str(int* v, int w, bool left_just, char* r)
{
...
template <int N> inline unsigned iv_leading_bits(const int* op, bool bit);
...
public:
  ...
  iv() {}
  template <int N2> iv(const iv<N2>& b)
  {
    const int M = AC_MIN(N, N2);
    iv_copy<M>(b.v, v);
    iv_extend<N - M>(v + M, (v[M - 1] < 0) ? ~0 : 0);
  }
  iv(Slong t) { iv_assign_int64<N>(v, t); }
  iv(Ulong t) { iv_assign_uint64<N>(v, t); }
  iv(int t)
  {
    v[0] = t;
  ...
  template <int N2, int Nr> inline void mult(const iv<N2>& op2, iv<Nr>& r) const { iv_mult<N, N2, Nr>(v, op2.v, r.v); }
  template <int N2, int Nr> void add(const iv<N2>& op2, iv<Nr>& r) const { iv_add<N, N2, Nr>(v, op2.v, r.v); }
  template <int N2, int Nr> void sub(const iv<N2>& op2, iv<Nr>& r) const { iv_sub<N, N2, Nr>(v, op2.v, r.v); }
  template <int Num_s, int N2, int Den_s, int Nr> void div(const iv<N2>& op2, iv<Nr>& r) const
  {
    iv_div<N, Num_s, N2, Den_s, Nr>(v, op2.v, r.v);
  }
  template <int Num_s, int N2, int Den_s, int Nr> void rem(const iv<N2>& op2, iv<Nr>& r) const
  {
    iv_rem<N, Num_s, N2, Den_s, Nr>(v, op2.v, r.v);
  }
  void increment() { iv_uadd_carry<N>(v, true, v); }
  void decrement() { iv_sub_int_borrow<N>(v, 0, true, v); }
  template <int Nr> void neg(iv<Nr>& r) const { iv_neg<N, Nr>(v, r.v); }
  template <int Nr> void shift_l(unsigned op2, iv<Nr>& r) const { iv_shift_l<N, Nr>(v, op2, r.v); }
  template <int Nr> void shift_l2(signed op2, iv<Nr>& r) const { iv_shift_l2<N, Nr, true>(v, op2, r.v); }
  template <int Nr> void shift_r(unsigned op2, iv<Nr>& r) const { iv_shift_r<N, Nr>(v, op2, r.v); }
  template <int Nr> void shift_r2(signed op2, iv<Nr>& r) const { iv_shift_r2<N, Nr, true>(v, op2, r.v); }
  template <int Nr, int B> void const_shift_l(iv<Nr>& r) const { iv_const_shift_l<N, Nr, B>(v, r.v); }
  template <int Nr, int B> void const_shift_r(iv<Nr>& r) const { iv_const_shift_r<N, Nr, B>(v, r.v); }
  template <int Nr> void bitwise_complement(iv<Nr>& r) const { iv_bitwise_complement<N, Nr>(v, r.v); }
  template <int N2, int Nr> void bitwise_and(const iv<N2>& op2, iv<Nr>& r) const
  {
    iv_bitwise_and<N, N2, Nr>(v, op2.v, r.v);
  }
  template <int N2, int Nr> void bitwise_or(const iv<N2>& op2, iv<Nr>& r) const
  {
    iv_bitwise_or<N, N2, Nr>(v, op2.v, r.v);
  }
  template <int N2, int Nr> void bitwise_xor(const iv<N2>& op2, iv<Nr>& r) const
  {
    iv_bitwise_xor<N, N2, Nr>(v, op2.v, r.v);
  }
  template <int N2> bool equal(const iv<N2>& op2) const { return iv_equal<N, N2>(v, op2.v); }
  template <int N2> bool greater_than(const iv<N2>& op2) const { return iv_compare<N, N2, true>(v, op2.v); }
  template <int N2> bool less_than(const iv<N2>& op2) const { return iv_compare<N, N2, false>(v, op2.v); }
...
template <> inline Slong iv<1>::to_int64() const
{
  return v[0];
}
...
template <typename T> struct c_prom {
...
template <typename T> inline const char* c_type_name()
{
  return "unknown";
}
template <> inline const char* c_type_name<bool>()
{
  return "bool";
}
template <> inline const char* c_type_name<char>()
{
  return "char";
}
...
template <> inline const char* c_type_name<int>()
{
  return "int";
}
template <> inline const char* c_type_name<unsigned>()
{
  return "unsigned";
}
...
template <> inline const char* c_type_name<float>()
{
  return "float";
}
template <> inline const char* c_type_name<double>()
{
  return "double";
}
...
#pragma builtin
  ...
  inline void bit_adjust()
  {
    ...
    Base::v[N - 1]     = S ? ((Base::v[N - 1] << rem) >> rem) : (rem ? ((unsigned)Base::v[N - 1] << rem) >> rem : 0);
  }

  inline bool is_neg() const { return S && Base::v[N - 1] < 0; }

  // returns false if number is denormal
  template <int WE, bool SE> bool normalize_private(ac_int<WE, SE>& exp, bool reserved_min_exp = false)
  {
    int expt              = exp;
...
#endif
  template <ac_special_val V> inline ac_int& set_val()
  {
    const unsigned int all_ones = (unsigned)~0;
    if (V == AC_VAL_DC) {
      ac_int r;
...
#pragma clang diagnostic pop
  ...
  inline long to_long() const { return ac_private::long_w == 32 ? (long)Base::v[0] : (long)Base::to_int64(); }
  ...
  // Arithmetic : Binary ----------------------------------------------------
  template <int W2, bool S2> typename rt<W2, S2>::mult operator*(const ac_int<W2, S2>& op2) const
  {
    typename rt<W2, S2>::mult r;
    Base::mult(op2, r);
    return r;
  }
  template <int W2, bool S2> typename rt<W2, S2>::plus operator+(const ac_int<W2, S2>& op2) const
  {
    typename rt<W2, S2>::plus r;
    Base::add(op2, r);
    return r;
  }
  template <int W2, bool S2> typename rt<W2, S2>::minus operator-(const ac_int<W2, S2>& op2) const
  {
...
#pragma GCC diagnostic ignored "-Wenum-compare"
#endif
  template <int W2, bool S2> typename rt<W2, S2>::div operator/(const ac_int<W2, S2>& op2) const
  {
  ...
  template <int W2, bool S2> typename rt<W2, S2>::mod operator%(const ac_int<W2, S2>& op2) const
  {
...
#endif
  // Arithmetic assign  ------------------------------------------------------
  template <int W2, bool S2> ac_int& operator*=(const ac_int<W2, S2>& op2)
  {
    Base r;
    Base::mult(op2, r);
    Base::operator=(r);
...
#pragma GCC diagnostic pop
  ...
  template <int W2, bool S2> typename rt<W2, S2>::logic operator&(const ac_int<W2, S2>& op2) const
  {
  ...
  template <int W2> ac_int operator<<(const ac_int<W2, true>& op2) const
  {
    ac_int r;
  ...
  template <int W2> ac_int operator<<(const ac_int<W2, false>& op2) const
  {
    ac_int r;
  ...
  // Shift assign ------------------------------------------------------------
  template <int W2> ac_int& operator<<=(const ac_int<W2, true>& op2)
  {
    Base r;
    Base::shift_l2(op2.to_int(), r);
    Base::operator=(r);
  ...
  template <int W2> ac_int& operator<<=(const ac_int<W2, false>& op2)
  {
    Base r;
    Base::shift_l(op2.to_uint(), r);
    Base::operator=(r);
  ...
  // Relational ---------------------------------------------------------------
  template <int W2, bool S2> bool operator==(const ac_int<W2, S2>& op2) const { return Base::equal(op2); }
  template <int W2, bool S2> bool operator!=(const ac_int<W2, S2>& op2) const { return !Base::equal(op2); }
  template <int W2, bool S2> bool operator<(const ac_int<W2, S2>& op2) const { return Base::less_than(op2); }
  template <int W2, bool S2> bool operator>=(const ac_int<W2, S2>& op2) const { return !Base::less_than(op2); }
  template <int W2, bool S2> bool operator>(const ac_int<W2, S2>& op2) const { return Base::greater_than(op2); }
  template <int W2, bool S2> bool operator<=(const ac_int<W2, S2>& op2) const { return !Base::greater_than(op2); }
...
#pragma builtin
  ...
  public:
    ...
    template <int W2, bool S2> operator ac_int<W2, S2>() const { return operator bool(); }
    ...
    template <int W2, bool S2> inline ac_bitref operator=(const ac_int<W2, S2>& val) { return operator=(val.to_int()); }
    inline ac_bitref operator                           =(const ac_bitref& val) { return operator=((int)(bool)val); }
  };
  ...
  template <int Na> inline void bit_fill(const int (&ivec)[Na], bool bigendian = true)
  {
...
template <> inline ac_int<1, true>::ac_int(bool b)
{
  v[0] = b ? -1 : 0;
}

template <> inline ac_int<1, false>::ac_int(bool b)
{
  v[0] = b;
}
template <> inline ac_int<1, false>::ac_int(signed char b)
{
  v[0] = b & 1;
}
...
template <> inline ac_int<1, false>::ac_int(signed short b)
{
  v[0] = b & 1;
}
...
template <> inline ac_int<1, false>::ac_int(signed int b)
{
  v[0] = b & 1;
}
template <> inline ac_int<1, false>::ac_int(unsigned int b)
{
  v[0] = b & 1;
}
template <> inline ac_int<1, false>::ac_int(signed long b)
{
  v[0] = b & 1;
}
...
template <> inline ac_int<1, false>::ac_int(Ulong b)
{
  v[0] = (int)b & 1;
}
template <> inline ac_int<1, false>::ac_int(Slong b)
{
  v[0] = (int)b & 1;
}

template <> inline ac_int<8, true>::ac_int(bool b)
{
  v[0] = b;
}
template <> inline ac_int<8, false>::ac_int(bool b)
{
  v[0] = b;
}
template <> inline ac_int<8, true>::ac_int(signed char b)
{
  v[0] = b;
}
template <> inline ac_int<8, false>::ac_int(unsigned char b)
{
  v[0] = b;
}
...
template <> inline ac_int<16, true>::ac_int(bool b)
{
  v[0] = b;
}
template <> inline ac_int<16, false>::ac_int(bool b)
{
  v[0] = b;
}
template <> inline ac_int<16, true>::ac_int(signed char b)
{
  v[0] = b;
}
template <> inline ac_int<16, false>::ac_int(unsigned char b)
{
  v[0] = b;
}
template <> inline ac_int<16, true>::ac_int(unsigned char b)
{
  v[0] = b;
}
...
template <> inline ac_int<16, false>::ac_int(unsigned short b)
{
  v[0] = b;
}
...
template <> inline ac_int<32, true>::ac_int(signed int b)
{
  v[0] = b;
}
template <> inline ac_int<32, true>::ac_int(unsigned int b)
{
  v[0] = b;
}
...
template <> inline ac_int<32, true>::ac_int(Slong b)
{
  v[0] = (int)b;
}
template <> inline ac_int<32, true>::ac_int(Ulong b)
{
  v[0] = (int)b;
}
...
#define REL_OP_WITH_INT(REL_OP, C_TYPE, W2, S2)                                                                        \
  template <int W, bool S> inline bool operator REL_OP(const ac_int<W, S>& op, C_TYPE op2)                             \
  {                                                                                                                    \
    return op.operator REL_OP(ac_int<W2, S2>(op2));                                                                    \
  }                                                                                                                    \
  template <int W, bool S> inline bool operator REL_OP(C_TYPE op2, const ac_int<W, S>& op)                             \
  {                                                                                                                    \
    return ac_int<W2, S2>(op2).operator REL_OP(op);                                                                    \
  }

#define ASSIGN_OP_WITH_INT(ASSIGN_OP, C_TYPE, W2, S2)                                                                  \
  template <int W, bool S> inline ac_int<W, S>& operator ASSIGN_OP(ac_int<W, S>& op, C_TYPE op2)                       \
  {                                                                                                                    \
...
template <typename T> inline T bit_fill_hex(const char* str)
{
  T res;
...
template <typename T, int W, bool S> T* operator+(const ac_int<W, S>& op2, T* ptr)
{
  return ptr + op2.to_int64();
}
...
template <ac_special_val V, int W, bool S> inline ac_int<W, S> value(ac_int<W, S>)
{
  ac_int<W, S> r;
...
template <ac_special_val V, int W, bool S> inline bool init_array(ac_int<W, S>* a, int n)
{
