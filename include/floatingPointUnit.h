...
private:
  ...
  enum roundingMode { RNE = 0, RTZ = 1, RDN = 2, RUP = 3, RMM = 4 };
...
public:
  ...
  bool process(struct DCtoEx dctoEx, ac_int<BIT_W, false>& result, bool& stall)
  {
    ...
    if ((dctoEx.opCode == RISCV_FLOAT_MADD) | (dctoEx.opCode == RISCV_FLOAT_MSUB) |
        (dctoEx.opCode == RISCV_FLOAT_NMADD) | (dctoEx.opCode == RISCV_FLOAT_NMSUB)) {
      /*
      FMADD
              step n°0 : rs1 * rs2
    ...
    if (dctoEx.useRs2 &
        (dctoEx.opCode == RISCV_FLOAT_OP & dctoEx.funct7 != RISCV_FLOAT_OP_CMP &
         ((f1UExp == MAX_EXP & f1Mantissa.slc<S_MTS>(0) != 0) |
          (f2UExp == MAX_EXP &
           f2Mantissa.slc<S_MTS>(0) != 0)))) // we have a float instructions at two operand with at least a Nan
    {
      changeResult = true;
      if (((f1UExp == MAX_EXP & f1Mantissa.slc<S_MTS>(0) != 0) &
           (f2UExp == MAX_EXP & f2Mantissa.slc<S_MTS>(0) != 0))) // both are Nan
        localResult = CNAN;
      ...
      if (performFused | performSqrt) {
        ...
        tempValue = localResult;
               ...
               ((f1UExp == MAX_EXP & f1Mantissa.slc<S_MTS>(0) != 0) |
                (f2UExp == MAX_EXP & f2Mantissa.slc<S_MTS>(0) !=
                                         0))) // CMP case we return false all the time if one of the operand is a Nan
    {
      changeResult = true;
      localResult  = 0;
    }
    ...
    if (((dctoEx.opCode == RISCV_FLOAT_OP) | (dctoEx.opCode == RISCV_FLOAT_MADD) | (dctoEx.opCode == RISCV_FLOAT_MSUB) |
         (dctoEx.opCode == RISCV_FLOAT_NMADD) | (dctoEx.opCode == RISCV_FLOAT_NMSUB))) {
      // Rounding mode
      switch (RNDM) {
        case RNE:
          ...
          if (roundingFlags[1] != 0 &
              localResult[0]) // tie to even, so if we are already even we change nothing, else we take the successor
            iterMantissa(localResult);
          break;
