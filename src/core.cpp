...
void forwardUnit(ac_int<5, false> decodeRs1, bool decodeUseRs1, ac_int<5, false> decodeRs2, bool decodeUseRs2,
                 ac_int<5, false> decodeRs3, bool decodeUseRs3, ac_int<5, false> executeRd, bool executeUseRd,
                 bool executeIsLongComputation, ac_int<5, false> memoryRd, bool memoryUseRd,
                 ac_int<5, false> writebackRd, bool writebackUseRd, bool stall[5], struct ForwardReg& forwardRegisters)
{
             ...
             bool globalStall)
{
  ...
  ac_int<MEMORY_INTERFACE_SIZE * 8, false> nextInst;
  ac_int<32, false> multResult = 0, expectResult = 0;

  if (!localStall && !core.stallDm)
    core.im->process(core.pc, WORD, LOAD, 0, nextInst, core.stallIm);

  fetch(core.pc, ftoDC_temp, nextInst.slc<32>(0));
  ...
  bool expectUsed = core.expectALU.process(core.dctoEx, expectResult, core.stallAlu); // calling expectALU: execute
                                                                                      // stage
  ...
  if (expectUsed)
    extoMem_temp.result = expectResult;
  ...
  if (!core.stallSignals[STALL_MEMORY] && !localStall && memtoWB_temp.we && !core.stallIm) {
    ...
    ac_int<MEMORY_INTERFACE_SIZE * 8, false> memVal = 0;
    memVal.set_slc(0, memtoWB_temp.result);
    core.dm->process(memtoWB_temp.address, mask, memtoWB_temp.isLoad ? LOAD : (memtoWB_temp.isStore ? STORE : NONE),
                     memtoWB_temp.valueToWrite, memVal, core.stallDm);
    memtoWB_temp.result = memVal.slc<32>(0);
  }
...
{
  Core core;
  IncompleteMemory<MEMORY_INTERFACE_SIZE> imInterface = IncompleteMemory<MEMORY_INTERFACE_SIZE>(imData);
  IncompleteMemory<MEMORY_INTERFACE_SIZE> dmInterface = IncompleteMemory<MEMORY_INTERFACE_SIZE>(dmData);
