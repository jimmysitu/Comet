...
void setTest(struct processorState& initialState, struct processorState& finalState, unsigned int& instruction,
             int opCode, int funct7, int funct3, int* a, int* b, int* z)
{
  float fa, fb, fc, fz;
  float* c;
  int d;
...
int Test(int opCode, int funct7, int funct3, int* a, int* b, int* z)
{
  ...
  if ((int)core.regFile[2] != (int)finalState.regs[2]) {
    ...
    printf("Issue with instruction : %x at register 2, awnser is %x and should be %x\n", instruction,
           (unsigned int)core.regFile[2], finalState.regs[2]);
  }
  ...
  if (!((diff == 0) | (val1 ^ val2 < 2))) {
    ...
    printf("Issue with instruction : %x at register 34, awnser is %x and should be %x\n", instruction,
           (unsigned int)core.regFile[34], finalState.regs[34]);
  }
  return c;
}
